...
class CenterNet3DLoss(Criterion):
    ...
    def loss(self, pred, target, stride):
        ...
        return category_loss, attribute_loss, offset_loss, depth_loss, size_loss, rotation_loss, velocity_loss, dir_loss

    def forward(self, target, pred):
        total_loss = 0
        loss_log = {}
        for stride in target.keys():
            category_loss, attribute_loss, offset_loss, depth_loss, size_loss, rotation_loss, velocity_loss, dir_loss = self.loss(pred, target, int(stride))
            loss = category_loss+attribute_loss+(offset_loss+0.2*depth_loss+size_loss+rotation_loss+0.05*velocity_loss)+dir_loss
            total_loss+=loss

...
#                 self.model = DistributedDataParallel(self.model)
    ...
    def create_model(self,):
        if self.config.model.model_name=='mobilenet':
            model = CenterNet3D(feature_extractor=MobileNetv2(self.config.device, pretrained=True), num_cate=len(self.meta_data['categories']), num_attr=len(self.meta_data['attributes']))
        ...
        #     model = EfficientNet.from_pretrained(self.config.model.model_name, num_classes=self.config.model.num_class)
            ...
            model = CenterNet3D(feature_extractor=ResNet101(self.config.device, pretrained=True), num_cate=len(self.meta_data['categories']), num_attr=len(self.meta_data['attributes']))
        elif self.config.model.model_name=='resnet101_dcn':
            model = CenterNet3D(feature_extractor=ResNet101DCN(), num_cate=len(self.meta_data['categories']), num_attr=len(self.meta_data['attributes']))
        else:
...
class CenterNet3DTransformer():
    def __init__(self, config):
        self.config = config
        self.meta_data = pickle.load(open(config.data.meta_data, 'rb'))
        
    def transform_predict(self, pred, det_thres=0.5, nms_thres=0.3):
        boxes = []

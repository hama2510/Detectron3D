...
class CenterNetDepth3D(nn.Module):
    def __init__(self, feature_extractor, fpn, num_cate, num_attr, num_depth=50):
        super().__init__()
        self.feature_extractor = feature_extractor
        self.fpn = fpn(self.feature_extractor.channel_num, 256)
        self.cls_head = ClassificationHead(
            256, num_cate, num_attr, num_depth, (3, 3), 1, 1
        )
...
class ClassificationHead(nn.Module):
    def __init__(
        self, in_channel, num_cate, num_attr, num_depth, kernel_size, padding, num_conv
    ):
        super().__init__()
        self.relu = nn.ReLU()

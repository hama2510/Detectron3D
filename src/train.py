...
if __name__ == '__main__':
    ...
    for model_id, item in enumerate(config.models):
        ...
        models.append({'model':model, 'optimizer':optimizer, 'config':model_config, 'y_true':[], 'y_pred':[], 'best_f1':0, 'loss':[]})
        ...
        f.write('epoch,loss,f1,precision,recall\n')
        f.close()
    ...
    for epoch in range(1, config.epochs+1):
        ...
        for step, samples in enumerate(tqdm(dataloader_train, desc="Train", leave=False)):
            for model_id in range(0, len(models)):
                ...
                loss.backward()
                optimizer.step()
                models[model_id]['loss'].append(loss.cpu().detach().numpy())
        ...
        for model_id in range(0, len(models)):
            ...
            f.write('{},{},{},{},{}\n'.format(epoch, np.mean(models[model_id]['loss']), f1, precision, recall))
            f.close()
